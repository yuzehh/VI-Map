...
from .utils import cam_to_pixel, get_rot_2d, plane_grid_2d
...
class ViewTransformation(nn.Module):
    def __init__(self, fv_size, bv_size, n_views=6):
        ...
        for i in range(self.n_views):
            fc_transform = nn.Sequential(
                nn.Linear(int(fv_dim), int(bv_dim)),
                nn.ReLU(),
                nn.Linear(int(bv_dim), int(bv_dim)),
                nn.ReLU()
            )
    ...
    def forward(self, feat):
        ...
        outputs = []
        for i in range(N):
            output = self.hw_mat[i](feat[:, i]).view(B, C, int(self.bv_size[0]), int(self.bv_size[1]))
...
class HDMapNet(nn.Module):
    ...
    def forward(self, img, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll):
        ...
        # TODO: discuss forward

    # def forward(self, img, car_trans, yaw_pitch_roll):
    #     x = self.get_cam_feats(img)
